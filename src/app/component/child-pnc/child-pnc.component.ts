...
//*************************************************************Parent Hierarchy******************************************* */
  ...
  submitForm(childPNCForm) {
    ...
    let data = {
      ...
      referralFacilityIdInfant: Number(
        childPNCForm.value.referralFacilityIdInfant
      ),
  ...
  saveChildPNCData(data: any): void {
    ...
    this.backendApiService.saveChildPNC(data).subscribe(
      (res) => {
        ...
        console.log(
          'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb' + this.infantRegistration
        );
    ...
    if (e == '0') {
      ...
      if (this.infantPNC.length !== undefined) {
        dateMinimum = new Date(
          this.datepipe.transform(
            this.infantPNC[this.infantPNC.length - 1].pncDate,
            'yyyy-MM-dd'
          )
        );
        this.minDateDeath = {
        ...
        dateMinimum = new Date(
          this.datepipe.transform(this.chilDOB, 'yyyy-MM-dd')
        );
        this.minDateDeath = {
          year: dateMinimum.getUTCFullYear(),
          month: dateMinimum.getMonth() + 1,
