...
import { FormGroup, FormBuilder, FormControl, Validators, Form, AbstractControl, ValidatorFn } from '@angular/forms';
...
export class EcprofileComponent implements OnInit {
  ...
  searchHusbandname: string;
  // editableSearch:boolean=false; husband_NA_Search:boolean=false
  //************************************************************************************************************ */
  /*
  ...
  */
  ...
  searchECData(data: any): void {
    ...
    this.backendApiService.GetSearchUsersAPI(data).subscribe((res: Response) => {
      ...
      if (response.status == true) {
        ...
        if (this.searchArray[0].page_code == "EC") { this.pagestatus = 'Eligible Couple' }
        else if (this.searchArray[0].page_code == "ECT") { this.pagestatus = 'EC Tracking' }
        else if (this.searchArray[0].page_code == "MR") { this.pagestatus = 'Mother Registration' }
        else if (this.searchArray[0].page_code == "MA") { this.pagestatus = 'Mother ANC' }
        else if (this.searchArray[0].page_code == "MM") { this.pagestatus = 'Mother Medical' }
        else if (this.searchArray[0].page_code == "MD") { this.pagestatus = 'Mother Delivery' }
        else if (this.searchArray[0].page_code == "MI") { this.pagestatus = 'Mother Infant' }
        else if (this.searchArray[0].page_code == "MP") { this.pagestatus = 'Mother PNC' }
        else if (this.searchArray[0].page_code == "IP") { this.pagestatus = 'Infant PNC' }
        else {
        ...
        alert('No record found. You may continue with new registration');
        this.createForm();
        ...
        if (this.selectedVillage !== undefined) {
          ...
          if (data.name_husband == "" || data.name_husband == null) {//debugger
            this.disableHusband(false);
            //.husband_NA_Search=false
            /*  const hus = this.ECForm.controls['HusbandNotApplicable'].value;
              hus.checked = true; */
            this.ECForm.controls['HusbandNotApplicable'].setValue(true);
            this.theCheckbox = true
          } else {
            this.ECForm.controls['HusbandFirstname'].setValue(data.name_husband);
            this.searchHusbandname = data.name_husband
            // this.husband_NA_Search=true
            //  this.husband_NA_Search=false
            /*  const hus = this.ECForm.controls['HusbandNotApplicable'].value;
               hus.checked = false;  */
            this.ECForm.controls['HusbandNotApplicable'].setValue(false);
          }
        }
      }
  ...
  edit(e) {
    ...
    if (e.page_code == "EC") {
      ...
      window.localStorage.setItem("PageCode", "ECSEARCH")
  ...
  //************************************************************************************ */
  @ViewChild(SearchComponent) search_component_obj;
  ...
  FillProviderandECdeatils() {
    if (this.selectedVillage != undefined) {
      ...
      if (Number(window.localStorage.getItem("ECT-EC")) > 0) {
  ...
  private createForm() {
    this.ECForm = this.formBuilder.group({
      ...
      mobno: new FormControl('', [Validators.required, Validators.pattern('[6-9]{1}[0-9]{9}$'), Validators.maxLength(10), this.patternMobValidator()]),
      ...
      PWfirstname: new FormControl('', [Validators.required, Validators.pattern('[A-Za-z. ]+'), this.noWhitespaceValidator]),
      PWSecondname: new FormControl('', [Validators.pattern('[A-Za-z. ]+'), this.noWhitespaceValidator]),
      PWLastname: new FormControl('', [Validators.pattern('[A-Za-z. ]+'), this.noWhitespaceValidator]),
      ...
      PWBranchname: new FormControl({ value: '', disabled: false }, [Validators.pattern('^([.0-9a-zA-Z\s ,&])+$'), Validators.maxLength(50), this.noWhitespaceValidator]),
      ...
      HusbandNotApplicable: new FormControl(''),
      HusbandFirstname: new FormControl('', [Validators.pattern('[A-Za-z. ]+'), this.noWhitespaceValidator]),
      HusbandSecondname: new FormControl('', [Validators.pattern('[A-Za-z. ]+'), this.noWhitespaceValidator]),
      HusbandLastname: new FormControl('', [Validators.pattern('[A-Za-z. ]+'), this.noWhitespaceValidator]),
      ...
      HusbandBranch: new FormControl({ value: '', disabled: false }, [Validators.pattern('^([.0-9a-zA-Z\s ,&])+$'), Validators.maxLength(50), this.noWhitespaceValidator]),
  ...
  validation(ECForm) {
    ...
    //------------------------Youngest Child Age & Sex----------------

    if ((this.ECForm.controls['MaleLive'].value != 0 || this.ECForm.controls['FemaleLive'].value != 0 || this.ECForm.controls['OtherLive'].value != 0) && (this.ECForm.controls['YoungestChildSex'].value != '')) {
      console.log(this.ECForm.controls['YoungestChildYear'].value)
      console.log(this.ECForm.controls['YoungestChildMonth'].value)
      console.log(this.ECForm.controls['YoungestChildSex'].value)
      if (this.ECForm.controls['YoungestChildYear'].value == "" || this.ECForm.controls['YoungestChildMonth'].value == "") {
        this.validationMsg = true;
        this.vmsg.push('Enter Youngest Child Details');
        return;
      }
    }
  ...
  onSubmit(ECForm) {
    ...
    if (this.ECForm.controls['RCHID'].value == "") {
      ...
      data.nameWife = (this.ECForm.controls['PWfirstname'].value).trim();

      data.middleNameWife = (this.ECForm.controls['PWSecondname'].value).trim();
      data.lastNameWife = (this.ECForm.controls['PWLastname'].value).trim();
      data.aadharNo = 0
      if (this.PWAccountDisabled == false) {
        ...
        data.pwBranchName = (this.ECForm.controls['PWBranchname'].value).trim();
      ...
      if (this.marked = true) {
        data.nameHusband = (this.ECForm.controls['HusbandFirstname'].value).trim();
        data.middleNameHusband = (this.ECForm.controls['HusbandSecondname'].value).trim();
        data.lastNameHusband = (this.ECForm.controls['HusbandLastname'].value).trim();
        ...
        if (this.AccountNoDisabled == false) {
          ...
          data.husbandBranchName = (this.ECForm.controls['HusbandBranch'].value).trim();
      ...
      if (this.ECForm.controls['Address'].value == "") { alert("Enter Address") } else {
        data.address = (this.ECForm.controls['Address'].value).trim();
      }
      ...
      if (this.ECForm.controls['YoungestChildYear'].value == '') {
        data.youngChildAgeYear = null
        data.youngChildAgeMonth = null
      } else {
  ...
  enableHusband(isedit?: boolean) {
    ...
    //AccountNoDisabled
    if (this.ECForm.controls['HusbandAccountNotAvailable'].value == false) {
  ...
  toggleVisibility(e) {
    ...
    if (e.target.checked === true) {
      ...
      this.enableHusband(false);
      if (this.searchHusbandname != null) {
        //alert(this.searchHusbandname)
        this.ECForm.controls['HusbandFirstname'].setValue(this.searchHusbandname);
      ...
      if (this.ECForm.controls['PWage'].value != "") {
        if (this.ECForm.controls['PWage'].value < 10 || this.ECForm.controls['PWage'].value > 60) {
          ...
          if (this.ECForm.controls['PWage'].value > 49) {
            ...
            alert('Beneficiary age entered is greater than 49 years');
          }
  ...
  onreset() {
    ...
    this.fill_hierarchy = true
    this.search_component_obj.onreset();
    this.btncontinue = false
    this.gen_rchid = 0
  }
  ...
  onContinue() {
    ...
    if (this.gen_rchid > 0) {
      ...
      window.localStorage.setItem("PageCode", "ECPROFILE")
  ...
  postECData(data: any): void {
    ...
    this.backendApiService.postECData(data).subscribe(res => {
      ...
      if (response.status == 409) {
        ...
        alert("Beneficiary record with same  Name, Husband Name and Mobile number already exist")
      }
      else {
  ...
  bindformEC(ec: ECModel) {
    ...
    if (this.Editmode == true) {
      ...
      this.gen_rchid = Number(ec.registrationNo);
      this.getectdetails(ec.registrationNo)
      this.getParentHierarchy();
      //alert(this.gen_rchid)
      /*   if (this.selectedSubCentre != ec.healthSubFacilityCode) {
          this.getHealthProviderByANMType(ec.healthSubFacilityCode, 2)
        this.getHealthProviderByASHAType(ec.healthSubFacilityCode, 1)
        this.getHealthProviderByMPWType(ec.healthSubFacilityCode, 5)
        } */
      ...
      if (this.RegdateUpdate == true) {
        this.ECForm.controls['RegDate'].disable()
      }
      else {
        this.ECForm.controls['RegDate'].enable()
      }
      ...
      if (ec.nameHusband == "" || ec.nameHusband == null) {
        this.ECForm.controls['HusbandNotApplicable'].setValue(true)
        this.disableHusband(true)


        this.ECForm.controls['HusbandAge'].setValue('');
        this.whosemobile.forEach((item, index) => {
          if (item.id === 'H') this.whosemobile.splice(index, 1);
        });
      }
      else {

        this.enableHusband(true)
      ...
      if (this.whosemobile.length > 0) {
        this.ECForm.controls['WhoseMobile'].setValue(ec.whoseMobile);
      }
      else {
        if (ec.nameHusband == "" || ec.nameHusband == null) {
          // this.whosemobile.slice(a=>a.id==='H')
          this.whosemobile.forEach((item, index) => {
            if (item.id === 'H') this.whosemobile.splice(index, 1);
          });
          // this.whosemobile = [{ id: 'W', whosemobile: 'Self' }, { id: 'N', whosemobile: 'Neighbour' }, { id: 'R', whosemobile: 'Relative' }, { id: 'O', whosemobile: 'Other' }];
      ...
      if (ec.youngChildAgeYear == null || (ec.youngChildAgeYear == 0 && ec.youngChildAgeMonth == 0)) {
        ...
        if (ec.maleChildLive > 0 && ec.femaleChildLive > 0) {
          this.ECForm.controls['YoungestChildSex'].enable();
        }
        else {
          this.ECForm.controls['YoungestChildSex'].disable();
  ...
  getectdetails(registrationId: number) {
    this.backendApiService.getECTbyRegNo(registrationId).subscribe((res: Response) => {
      ...
      if (response.length > 0) {
        this.RegdateUpdate = true
      }
      else { this.RegdateUpdate = false }
    })
  ...
  getParentHierarchy() {
    this.backendApiService.getStatebyID(this.parentState).subscribe((res: Response) => {
      this.parentStateName = JSON.parse(JSON.stringify(res)).nameG;
    })

    this.backendApiService.getDistrictbyID(this.parentDistrict).subscribe((res: Response) => {
      this.parentDistrictName = JSON.parse(JSON.stringify(res))[0].name;

    })

    this.backendApiService.getTalukabyID(this.parentDistrict).subscribe((res: Response) => {
      this.parentTalukaName = JSON.parse(JSON.stringify(res))[0].nameG;
    })

    this.backendApiService.getBlockbyID(this.parentBlock).subscribe((res: Response) => {
      this.parentBlockName = JSON.parse(JSON.stringify(res))[0].name;
    })

    this.backendApiService.getFacilitybyID(this.parentFacility).subscribe((res: Response) => {
      this.parentFacilityName = JSON.parse(JSON.stringify(res))[0].name;
    })

    this.backendApiService.getSubcenterbyID(this.parentSubcenter).subscribe((res: Response) => {
      this.parentSubcenterName = JSON.parse(JSON.stringify(res))[0].name;
