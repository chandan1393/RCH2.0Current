...
//  bootstrep model popup code start here for view more	
	   ...
	   if(popupname=='Approve')
	   {
		  ...
		  this.funSendLoginDetailsEmail(user.email,user.id,user.userName);
  ...
  userApproved(userId,statusid,approvedBy,ipAddress,username,reason): void {
   ...
   funSendLoginDetailsEmail(email:string,id:string,userName:string)
{
	 this.backendApiService.SendLoginDetailsEmail(email,id,userName).subscribe((res:Response)=> {
...
//	alert(response.otp);
	 ...
	 console.log('error in funSendLoginDetailsEmail function');
	 console.log(error);
